package com.demo.rent.controller;

import java.time.ZonedDateTime;
import java.util.Iterator;

import com.demo.rent.request.PriceRequest;
import com.demo.rent.response.PriceResponse;
import com.demo.rent.response.RentChargesResponse;
...
public class RentController {
    ...
    public ResponseEntity getPossibleCharges(@PathVariable("id") String id) {
        ...
        ZonedDateTime endTime = startTime.plusDays(2);

        HttpEntity<PriceRequest> request = new HttpEntity<>(new PriceRequest("vip", startTime, endTime));
        PriceResponse response = restTemplate.postForObject(instance.getHomePageUrl() + "/v1/prices", request,
                PriceResponse.class);
